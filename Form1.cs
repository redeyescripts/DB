...
namespace RecipeDB
{
    public partial class CookBook : Form
    {
        ...
        {
            string query = "SELECT Ingredient.Name FROM Ingredient INNER JOIN RecipeIngredient ON Igredient.Id = RecipeIngredient.IngredientId WHERE RecipeIngredient.RecipeId = @RecipeId";
            using(connection = new SqlConnection(connectionString))
